...
public class BoardAuthenticationFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		//수정, 삭제 요청이 들어올 때는 작성자(writer) 정보를 넘겨준다
		...
		if(id == null) {
			...
			return; //함수 종료해주지 않으면 -> 컨트롤러를 실행하지 않음
			
		}
		
		//클라이언트에서 넘어오는 id값과 세션id값을 비교
		String writer = request.getParameter("writer"); //현재 세션의 출입자와 원래 작성자 비교
		
		
		//작성자가 null 이거나, 세션값과 작성자가 같지 않다면
